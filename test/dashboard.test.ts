...
describe("dashboard", () => {
  ...
  describe("GET /", () => {
    test("returns HTML content-type", async () => {
      ...
      expect(text).toContain('aria-label="团队关注事项"')
      expect(text).toContain('aria-label="智能体列表"')
      expect(text).toContain('aria-label="任务看板"')
      expect(text).toContain('aria-label="活动动态"')
      expect(text).toContain('aria-label="事件时间线"')
  ...
  describe("unknown routes", () => {
    test("returns 404", async () => {
      ...
      expect(await res.text()).toBe("未找到页面")
    })
  })
